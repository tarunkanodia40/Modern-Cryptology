...
void shift_keys_enc_dec(int sw1, BYTE *CD, int r) 
{
    // INT sw1; /* type of cryption 0= encryption, 1= decryption */
    // BYTE *pkey;
    // INT r;
...
void find_password(BYTE key[],char password[]){
    shift_keys_enc_dec(1,key,6);
    char pass_l[25],pass_r[25];
    BYTE pass_byte_l[100], pass_byte_r[100],output_l[100],output_r[100];
    for (int i = 0; i < 16; i++) {
        pass_l[i] = password[i];
        pass_r[i] = password[i+16];
    }

    tobyte(pass_l, pass_byte_l);
    tobyte(pass_r, pass_byte_r);
    
    des(pass_byte_l, output_l, 6, 'N');
    des(pass_byte_r, output_r, 6, 'N');
    printf("Password found!! The password is:");
    for (int i = 0; i < 8; ++i)
    {
        printf("%c",output_l[i]);
    }
    ...
    printf("\n");
    printf("Removing Padding...Final password: ");
    for (int i = 0; i < 8; ++i)
    {
        printf("%c",output_l[i]);
    }
    printf("%c%c\n",output_r[0],output_r[1] );
    return;
}
int main(int argc, char **argv) {
    ...
    if (argc < 2) {
        printf("No input file containing all keys provided!\n");
        return 1;
    }
    ...
    char string[20] = "defghijklmnopqrs";
    char cipher[20] = "jspprgnpeniskpps";
    char password[] = "mqhrrlkikdprkinmjkjdikikfmdlreqf";
    ...
    while (fgets(line, sizeof line, fp)) {
        ...
        shift_keys_enc_dec(0, k, 6);
        des(input, outputs, 6, 'N');
        char *ans = tostr(outputs);
        if (strcmp(ans, cipher) == 0) {
            ...
            find_password(fin_key,password);
            exit(0);
        }
    }
    printf("Err.... Sorry not found!\n");
    fclose(fp);
    return 0;
}
